...
{
    return std::ranges::fold_left(range, initial, [&step](Result<State, E> acc, const auto& item) -> Result<State, E>
    {
        return acc.and_then([&](const State& state) { return step(state, item); });
    });
}
...
{
    return std::ranges::fold_left(range, initial, [&step](Status<E> acc, const auto& item) -> Status<E>
    {
        return acc.and_then([&] { return step(item); });
    });
}
