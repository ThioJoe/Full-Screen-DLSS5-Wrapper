...
{
    return interior::FrameState{ state.number, state.currentSet, state.states, state.hasOutput, state.hasPrevious, state.resetPending, state.zeroMotionWritten,
                                 state.lastCapture, state.display, infra::WithElement(state.slotFences, slot.Get(), value), state.statsPending, state.displaySource };
}
...
struct SessionStep
{
    ...
    {
        return env.BeginFrame(state).and_then([&](const auto& start)
        {
            return interior::PlanFrame(plan, state, start.input).transform_error([](interior::PlanFrameError e) { return Environment::FromPlanError(e); });
        });
    }
};

template <class Environment, class Error>
[[nodiscard]] infra::Result<SessionOutcome, Error> RunSession(Environment& env, const interior::SessionPlan& plan, interior::FrameState state,
                                                              std::uint64_t frameLimit) noexcept
{
    ...
    while (state.number.Get() < frameLimit)
    {
        ...
        const infra::Result<interior::FrameState, Error> next = env.Execute(*framePlan).transform([&](const auto& report)
        {
            return WithSlotFence(framePlan->next, interior::SlotOfFrame(state.number), report.signaled);
        });
        if (!next.has_value())
